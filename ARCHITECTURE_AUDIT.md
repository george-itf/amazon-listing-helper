...
| File | Purpose | Database Table | Classification |
|------|---------|----------------|----------------|
| `listing.repository.js` | Listing CRUD | `listings`, `listing_images` | **KEEP** |
| `score.repository.js` | Score CRUD | `scores` | **DEPRECATE** (legacy) |
| `alert.repository.js` | Alert CRUD | `alerts` | **KEEP** |
| `keepa.repository.js` | Keepa data CRUD | `keepa_data` | **REBUILD** (→ `keepa_snapshots`) |
| `task.repository.js` | Task CRUD | `tasks` | **KEEP** |
| `settings.repository.js` | Settings CRUD | `settings` | **KEEP** |
| `order.repository.js` | Order CRUD | `orders` (separate schema) | **KEEP** |
...
| `scores` | Scoring history | **DEPRECATE** - replaced by `feature_store` + `recommendations` |
| `alerts` | Alert notifications | Read/unread tracking |
| `keepa_data` | Keepa snapshots | **REBUILD** → `keepa_snapshots` with raw_json/parsed_json |
...
|------|---------|----------------|
| `credentials.json` | SP-API + Keepa API keys | **REBUILD** access via `CredentialsProvider` module |
| `listings.json` | Cached listing data | **REMOVE** (duplicates DB) |
| `scores.json` | Score history | **REMOVE** (duplicates DB; legacy scoring deprecated) |
...
### A.5 Credentials Access Pattern (Required Rebuild)

**Current State:** Multiple modules read `credentials.json` directly via `fs.readFileSync()`.
...
├─────────────────────────────────────────────────────────────────┤
│ - Single module that reads credentials.json                     │
│ - Exports: getSpApiCredentials(), getKeepaApiKey()             │
│ - All other code imports from CredentialsProvider              │
│ - No direct fs.readFileSync of credentials.json elsewhere      │
└─────────────────────────────────────────────────────────────────┘
```

**Files that currently read credentials directly (must be refactored):**
- `server.js` (SP-API initialization)
- `keepa-sync.js` (Keepa API key)
- `amazon-push.js` (SP-API calls)
- `orders-sync.js` (SP-API calls)

### A.6 Frontend (`main/web/`)
...
| Implementation | Location | Notes |
|----------------|----------|-------|
| `scoring.js` | 5-component scoring | **DEPRECATE** - replaced by feature_store |
| `score.repository.js` | Score persistence | **DEPRECATE** - replaced by feature_store |
...
**VERDICT:** Legacy scoring system (`scores` table, `scoring.js`, `score.repository.js`) is deprecated. New center is `feature_store` + `recommendations` tables per SPEC.
...
| `alert.repository.js` | Good patterns |
| `task.repository.js` | Good patterns |
| `settings.repository.js` | Good patterns |
| `order.repository.js` | Good patterns |
| Fastify server framework | Stable, performant |

### C.2 DEPRECATE (Legacy, Do Not Extend)

| Component | Reason | Replacement |
|-----------|--------|-------------|
| `scoring.js` | Legacy 5-component scoring | `feature_store` + `recommendations` |
| `score.repository.js` | Legacy score persistence | `feature_store` table |
| `scores` table | Legacy scoring history | `feature_store` + `recommendations` tables |
| `scores.json` | Legacy file-based scores | Remove entirely |

### C.3 REMOVE (Delete After Migration)
...
| **Feature computation** | None exists | New `feature_store` per SPEC |
| **Credentials access** | Direct file reads scattered | Single `CredentialsProvider` module |
| **All `/api/v1` endpoints** | Gradual migration | New `/api/v2` endpoints |

---

## D) Buy Box Signal Availability & Nullability

### D.1 What We Can Reliably Ingest Today

| Signal | Source | Reliability | Notes |
|--------|--------|-------------|-------|
| `buy_box_status` | SP-API GetCompetitivePricing or offer listing | **RELIABLE** | Enum: `WON`, `LOST`, `UNKNOWN` |
| `buy_box_percentage_30d` | SP-API Sales & Traffic Reports | **UNRELIABLE** | Often unavailable for FBM; may be null |
| `buy_box_price` | Keepa or SP-API | **RELIABLE** | Current Buy Box price (may differ from our price) |
| `is_buy_box_winner` | SP-API offer listing | **RELIABLE** | Boolean, derived to status |

### D.2 Signal Derivation Rules
...
buy_box_percentage_30d = NULL unless explicitly available from reports
```

### D.3 Nullability Contract

| Field | Nullable | Default | Behavior When Null |
|-------|----------|---------|-------------------|
| `buy_box_status` | NO | `'UNKNOWN'` | Always populated; `UNKNOWN` if cannot determine |
| `buy_box_percentage_30d` | YES | `NULL` | UI shows "N/A"; recommendations note limitation |

### D.4 Recommendation Behavior When Buy Box Signal Missing

When `buy_box_status = 'UNKNOWN'` or `buy_box_percentage_30d IS NULL`:

1. **PRICE_DECREASE_REGAIN_BUYBOX** recommendation:
   - Can still generate if `buy_box_status = 'LOST'`
   - If `buy_box_status = 'UNKNOWN'`, do not generate (insufficient signal)
   - `evidence_json.notes` must include: `"Buy Box status derived from latest offer sync; buy box % not available."`

2. **Buy Box risk calculation**:
   - If `buy_box_status = 'UNKNOWN'`: `buy_box_risk = 'UNKNOWN'` (not HIGH/MEDIUM/LOW)
   - UI must display "Buy Box: Unknown" distinctly from "Buy Box: Won" or "Buy Box: Lost"

3. **Feature store**:
   - `features_json.buy_box_status`: Always populated (`WON`/`LOST`/`UNKNOWN`)
   - `features_json.buy_box_percentage_30d`: May be `null`
   - `features_json.buy_box_risk`: May be `'UNKNOWN'` if status is unknown

---

## E) Proposed Cutover Strategy

### E.1 What Remains in `/api/v1` (Temporarily)
...
| **Slice C** | Job endpoints for sync, `GET /api/v2/jobs/*`, Feature store computation |
| **Slice D** | `GET /api/v2/recommendations`, Accept/Reject/Snooze endpoints |
| **Slice E** | `POST /api/v2/asins/analyze`, `GET /api/v2/asins/{asin_entity_id}`, Research pool endpoints |

### E.3 Data Migration Requirements
...
| `bom.json` | `boms` + `bom_lines` tables | Versioning starts at v1; set `is_active=true` |
...
| `keepa_data` rows | `keepa_snapshots` table | Transform to `raw_json` + `parsed_json` + `captured_at` |
| `scores` rows | **DO NOT MIGRATE** | Legacy; new features computed fresh |
...
| PostgreSQL Tables (current) | 7 |
| JSON Data Files | 12 |
| Redundant Subsystems | 7 |
| Components Marked REBUILD | 9 |
| Components Marked DEPRECATE | 4 |
| Components Marked REMOVE | 6 |
